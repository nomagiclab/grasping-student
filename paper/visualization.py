...
def predict(image, teacher, student, affordance):
    ...
    with torch.no_grad():
        def obtain_affordances(model=teacher, image=image):
            affordances = model.backbone.forward(image)
            affordances = copy.deepcopy(affordances)
            return affordances

        num_base_images = 1
        fig, (axes) = plt.subplots(num_base_images, 2)
        plt.tight_layout()

        if len(axes.shape) == 1:
            axes = axes.reshape(len(axes), 1)
        fig.set_size_inches(np.array((2 * 2 + 2, 2 * (num_base_images) + 1.5)) * 0.8)
        ...
        axes[0, 0].set_title(f'Teacher')
        axes[1, 0].set_title(f'Student')

        for i, affordances in enumerate([obtain_affordances(model=m, image=heightmap_depth) for m in [teacher, student]]):
            angle_idx = 0
            ...
            affordance_rotated *= 2
            affordance_rotated[affordance_rotated >= 1.] = 1.
            axes[i, 0].set_axis_off()

            # if i == 1:
            affordance_rotated = ndimage.maximum_filter(affordance_rotated, size=3)

            heightmap_color_rotated[..., 1] += affordance_rotated
            # ax[i].imshow(affordance_rotated, alpha=0.4)
            axes[i, 0].imshow(heightmap_color_rotated)
            # axes[i, 0].imshow(affordance_rotated)#, alpha=0.6)
            plt.tight_layout()
...
def main():
    ...
    teacher.backbone.num_rotations = 1
    ...
    student.backbone.num_rotations = 1
