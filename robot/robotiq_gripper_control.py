...
class RobotiqGripper(object):
    ...
    def open_async(self):
        """
        Open the gripper asynchronously.

        Returns:
            True if the command succeeded, otherwise it returns False
        """
        return self.call("OPEN_ASYNC", "rq_open()")
    ...
    def close_async(self):
        return self.call("CLOSE_ASYNC", "rq_close()")

    def detect_object(self) -> bool:
        return self.call("DETECT_OBJECT", "rq_is_object_detected()")
    ...
    def set_position(self, pos):
        assert type(pos) == int
        return self.call("SET_POSITION", f"rq_move_and_wait({pos})")
